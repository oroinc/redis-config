...
class SentinelSetup extends AbstractSetup
{
    ...
    public function getConfig(array $config)
    {
        ...
        $redisSentinelMasterName = $this->container->getParameter(
            sprintf(self::REDIS_SENTINEL_MASTER_NAME_PARAM, $redisClient)
        );
        $dsnParameterValues = $this->container->getParameter(sprintf('redis_dsn_%s', $redisClient));
        $dsns = [];
        $password = '';
        ...
        $config[$redisClient]['dsn'] = $dsns;
        $config[$redisClient]['options']['replication'] = self::TYPE;
        $config[$redisClient]['options']['service'] = $redisSentinelMasterName;
        $config[$redisClient]['options']['parameters']['database'] = $database;
        if ($password) {
            $config[$redisClient]['options']['parameters']['password'] = $password;
        }
